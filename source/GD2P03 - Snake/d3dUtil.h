...
D3DX10INLINE float Magnitude( const D3DXVECTOR3& _krA )
{
	return( sqrt( ( _krA.x * _krA.x ) + ( _krA.y * _krA.y ) + ( _krA.z * _krA.z ) ) );
}

D3DX10INLINE float Magnitude( const D3DXVECTOR2& _krA )
{
	return( sqrt( ( _krA.x * _krA.x ) + ( _krA.y * _krA.y ) ) );
}

D3DX10INLINE int GenerateRandom( int _iMin = 0, int _iMax = 1 )
{
	return ( rand() % ( _iMax - _iMin ) + _iMin );
}

D3DX10INLINE float AngleBetween( const D3DXVECTOR3& _krA, const D3DXVECTOR3& _krB )
{
	float fReturn = 0.0f;
	float fDotProd = D3DXVec3Dot( &_krA, &_krB );

	if( fDotProd != 0 )
	{
		float fMagA = Magnitude( _krA );
		float fMagB = Magnitude( _krB );
		float fTheta = fDotProd / ( fMagA * fMagB );
...
D3DX10INLINE float DistanceBetween( const D3DXVECTOR2& _krA, const D3DXVECTOR2& _krB )
{
	float fX = max( _krA.x, _krB.x ) - min( _krA.x, _krB.x );
	float fY = max( _krA.y, _krB.y ) - min( _krA.y, _krB.y );
	return ( sqrt( ( fX * fX ) + ( fY * fY ) ) );
}

D3DX10INLINE D3DXVECTOR2 Truncate( const D3DXVECTOR2& _krA, const float _fMax )
{
	D3DXVECTOR2 vReturn = _krA;

	if( Magnitude( _krA ) > _fMax )
	{
		D3DXVec2Normalize( &vReturn, &_krA );
		vReturn *= _fMax;
	}
